...
pub fn launch_virtual_dom_blocking(virtual_dom: VirtualDom, mut desktop_config: Config) {
    let mut custom_event_handlers = desktop_config
    ...
    event_loop
        .run(move |window_event, event_loop| {
            ...
            match window_event {
                ...
                Event::LoopExiting => app.handle_loop_destroyed(),
                Event::WindowEvent {
                    event, window_id, ..
                } => match event {
                    WindowEvent::CloseRequested => {
                        app.handle_close_requested(window_id, event_loop)
                    }
                    WindowEvent::Destroyed { .. } => app.window_destroyed(window_id, event_loop),
                ...
                Event::UserEvent(event) => match event {
                    ...
                    UserWindowEvent::CloseWindow(id) => app.handle_close_requested(id, event_loop),
                    ...
                    UserWindowEvent::HotReloadEvent(msg) => {
                        app.handle_hot_reload_msg(msg, event_loop)
                    }
